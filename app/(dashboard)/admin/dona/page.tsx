...
export default function AdminDonaPage() {
  ...
  const processedContacts = contacts.filter(c => c.status === 'processed').length;
  const pendingContacts = contacts.filter(c => c.status === 'pending').length;
  const totalContacts = contacts.length;

  const filteredContacts = contacts.filter(c => {
    if (filter === 'all') return true;
    if (filter === 'processed') return c.status === 'processed';
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-[#1E3A8A]">{totalContacts}</p>
                </div>
